...
- **`src/components/ResortTable.jsx`** — AG Grid wrapper. A `forwardRef` component that forwards `ref` to `<AgGridReact>`. No internal toolbar — all controls live in App.jsx's drag handle. Column data lives in `src/components/resortTableColumns.jsx` (`COLUMN_DEFS`, `HEADER_BY_FIELD`, `COL_GROUPS`, used by App.jsx's column-visibility popover), with `BoolCell`/`LinkCell` cell renderers split into `src/components/resortTableCells.jsx` — kept out of ResortTable.jsx itself so the file only exports the component, per `react-refresh/only-export-components`.
