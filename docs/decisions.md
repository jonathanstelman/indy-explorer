...
**Follow-up:** Add the weekly cron after a few successful manual runs. First real `workflow_dispatch` run still needs to be exercised end-to-end. (`GOOGLE_MAPS_API_KEY` was added to the repo's Actions secrets by the user on 2026-07-13, so geocoding works in CI.) Investigating the `full` input's real value surfaced a bug: the geocode step is all-or-nothing (skips entirely when `resort_locations.csv` exists; regenerates everything under `--full`), so new resorts never get city/state/country in incremental runs — filed as #134 (bugfix: incremental behavior was the original intent). Once #134 lands, the "full refresh ~quarterly" guidance mostly evaporates and the `full` input becomes a rarely-needed escape hatch.
