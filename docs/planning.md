...
`feature/138-140-141-tt-cleanup-affiliation-filter-lint` — **#138, #140, #141 implemented in series on one branch (user's explicit call — "reasonably easy to test independently of one another"), not yet PR'd.** Four commits: `f2f40ee` (#141 lint fixes), `29a5fb7` (#138 `_tt` field removal), `c45ad16` (#140 backend), `d72da2d` (#140 frontend + a regression fix). All tests pass (131 backend, 22 pipeline, 21 frontend), lint clean (0 errors, 6 pre-existing warnings), Black clean, verified live via Playwright. See `docs/decisions.md` for full per-issue detail. **Remaining: decide PR strategy (one combined PR vs. per-issue) and open it/them, then merge.**

**Notable:** #141's fix for one `setState`-in-effect warning (adding `urlValue` to a ref-sync effect's deps) broke every multi-select filter's commit-on-close app-wide (region/country/state, all Peak Rankings categorical filters) — `urlValue` is a fresh array reference every render, so the "fix" reran the effect every render and clobbered the in-progress selection. Caught via live Playwright testing of #140's new filter, confirmed by user report ("Peak Rankings dropdowns aren't hooked up"), fixed same-branch before considering #140 done. Full incident in `docs/decisions.md`'s #141/#140 entries.

**Next up (after this branch merges):** **#136** (data-changed pipeline verification, scheduled ~Nov 2026), older backlog **#21**/**#22**. No other open backlog remains from the recent polish pass.
